...
【条件】
- 現在地からスタートすること。
- 所要時間: 約${duration}分 (フォトウォークとして移動+撮影時間を含む) で回れること。
...
async function drawSmartRoute(routePoints) {
    ...
    try {
        ...
        if (data.routes && data.routes.length > 0) {
            ...
            // ★修正: 時速4kmで時間を計算 (OSRMのdurationは無視)
            ...
            renderRouteSidebar({ 
                ...
                distance: distMeters, 
                walkMinutes: walkMinutes // 計算済み時間を渡す
            });
